...
public class PaintingProgram extends JFrame {
	...
	Font creditFont = new Font("Berlin Sans FB Demi", Font.BOLD, 20);
	String creditStr = "~ Arjun Bawa   ";
	...
	JSlider transSlider, brushSlider;

	// Label for transparency, brush size, credits
	JLabel transLabel;
	JLabel brushLabel;
	JLabel credits;
	...
	// Transparency of the shape, brush size

	float transparentVal = 1.0f, brushSize = 5f;;
	...
	public PaintingProgram() {
		...
		ellipseBut = makeMeButtons("/Oval.png", 3);
		...
		strokeBut = makeMeColorButton("/Stroke.png", 5, true);
		fillBut = makeMeColorButton("/Fill.png", 6, false);
		...
		// Add the transparent, brush size label and sliders

		transLabel = new JLabel("Opacity: 100%");
		brushLabel = new JLabel("  5px");
		credits = new JLabel(creditStr);

		// Min value, Max value and starting value for slider

		transSlider = new JSlider(1, 100, 100);
		brushSlider = new JSlider(1, 100, 5);
		...
		brushSlider.addChangeListener(lForSlider);

		theBox.add(transLabel);
		theBox.add(transSlider);
		theBox.add(brushLabel);
		theBox.add(brushSlider);
	...
	public JButton makeMeButtons(String iconFile, final int actionNum) {
		...
		Icon butIcon = new ImageIcon(getClass().getResource(iconFile));
	...
	private class DrawingBoard extends JComponent {
			...
			this.addMouseMotionListener(new MouseMotionAdapter() {
				...
				public void mouseDragged(MouseEvent e) {
					...
					if (currentAction == 1) {
						...
						int x = (int) (e.getX() - brushSize / 2);
						int y = (int) (e.getY() - brushSize / 2);
						...
						aShape = drawBrush(x, y, brushSize, brushSize);

						shapes.add(aShape);
						shapeFill.add(fillColor);
						shapeStroke.add(strokeColor);

						// Add the transparency value, brush size
		...
		private Ellipse2D.Float drawBrush(int x1, int y1, float brushSize,
				float brushSize2) {

			return new Ellipse2D.Float(x1, y1, brushSize, brushSize2);
	...
	private class ListenForSlider implements ChangeListener {
		...
		public void stateChanged(ChangeEvent e) {
			...
			// Check if the source of the event was the transSlider
			...
			if (e.getSource() == transSlider) {
				...
				brushLabel.setText("  " + brushSlider.getValue() + "px");
				brushSize = (float) (brushSlider.getValue());
			}
